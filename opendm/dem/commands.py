...
def gap_fill(filenames, fout):
    ...
    # Use the other numpy function as signal.medfilt limited to 32 bit array pointers
    arr = ndimage.median_filter(arr,size=5,footprint=None,output=None,mode='nearest')
    
    # Fill corner points with nearest value
    # median_filter includes corner points so no longer requried
    #if arr.shape >= (4, 4):
